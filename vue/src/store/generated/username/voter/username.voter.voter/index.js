    ...
    actions: {
        ...
        async QueryRequestAttendance({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).queryRequestAttendance(key.id, { ...query, 'pagination.key': value.pagination.nextKey })).data;
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryRequestAttendance', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryRequestAttendanceAll({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).queryRequestAttendanceAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    for (let prop of Object.keys(next_values)) {
                        if (Array.isArray(next_values[prop])) {
                            value[prop] = [...value[prop], ...next_values[prop]];
                        }
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryRequestAttendanceAll', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryInformation({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).queryInformation(key.id, { ...query, 'pagination.key': value.pagination.nextKey })).data;
                    for (let prop of Object.keys(next_values)) {
                        if (Array.isArray(next_values[prop])) {
                            value[prop] = [...value[prop], ...next_values[prop]];
                        }
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryInformation', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryInformationAll({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).queryInformationAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    for (let prop of Object.keys(next_values)) {
                        if (Array.isArray(next_values[prop])) {
                            value[prop] = [...value[prop], ...next_values[prop]];
                        }
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryInformationAll', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryAttendance({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryAttendance', payload: { options: { all }, params: { ...key }, query } });
        ...
        async QueryAttendanceAll({ commit, rootGetters, getters }, { options: { subscribe = false, all = false }, params: { ...key }, query = null }) {
            try {
                ...
                while (all && value.pagination && value.pagination.nextKey != null) {
                    let next_values = (await (await initQueryClient(rootGetters)).queryAttendanceAll({ ...query, 'pagination.key': value.pagination.nextKey })).data;
                    for (let prop of Object.keys(next_values)) {
                        if (Array.isArray(next_values[prop])) {
                            value[prop] = [...value[prop], ...next_values[prop]];
                        }
                ...
                if (subscribe)
                    commit('SUBSCRIBE', { action: 'QueryAttendanceAll', payload: { options: { all }, params: { ...key }, query } });
